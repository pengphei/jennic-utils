/*
 * Copyright (c) 2015 Focalcrest, Ltd. All rights reserved.
 *
 * Author(s):
 *     Phil Han <phil@focalcrest.com>
 */

#include <stdio.h>
#include <string.h>

#include "util_crc.h"

/******************************************************************************
*
* Create an 8-bit crc table from the polynomial x^8 + x^2 + x + 1 ~ 0x07
*
******************************************************************************/
static unsigned char crc8_init = 0;
static unsigned char crc8_table[256];

unsigned char util_crc8(unsigned char* ucs, unsigned char ucslen, unsigned char crc)
{
    int ii = 0;

    if(0 == crc8_init)
    {
        int __ii = 0, __jj = 0;
        unsigned char __crc = 0;

        memset(crc8_table, 0, sizeof(crc8_table));

        for (__ii=0; __ii<256; __ii++)
        {
            __crc = __ii;
            for (__jj=0; __jj<8; __jj++)
            {
                __crc = (__crc << 1) ^ ((__crc & 0x80) ? 0x07 : 0);
            }
            crc8_table[__ii] = __crc & 0xFF;
        }

        crc8_init = 1;
    }

    for(ii = 0; ii < ucslen; ii++)
    {
        crc = crc8_table[crc ^ ucs[ii]];
    }

    return crc;
}

unsigned char util_crc8_by_table(unsigned char* ucs, unsigned char ucslen, unsigned char crc)
{
    int ii = 0;
    static const unsigned char crc8_array[256] =
    {
        ,
    };

    for(ii = 0; ii < ucslen; ii++)
    {
        crc = crc8_array[crc ^ ucs[ii]];
    }
    return crc;
}

int util_debug_buf(char* ptip, unsigned char* pbuf, int len)
{
    int ii = 0;

    printf("\n%s: \n", ptip);
    for(ii=0; ii<len; ii++)
    {
        printf("%02x ", pbuf[ii]);
        if(0 == len%8)
        {
            printf("\n");
        }
    }
    printf("\n\n");
    return 0;
}
